...
const maps_gen = @import("data/maps_gen.zig");
...
/// N-MAP · load a NAMED map's geometry into a world.
///
/// The `gen:N` counterpart is `world_state_generate_arena`; this is the
/// other half, for the maps that are hand-authored TS data
/// (`maps_gen.zig`, generated by `bun run gen:maps`). With both, a native
/// shell can construct every arena the browser can.
///
/// Writes statics + one_way into the state and the spawn points into the
/// module-level spawn table (same destination `world_state_set_spawn_points`
/// uses), so `world_init_roster` immediately afterwards places players
/// correctly. Returns the static count, or 0 for an unknown id — never a
/// substituted default map, because silently swapping geometry turns a
/// replay desync into a mystery instead of a failure.
pub export fn world_state_load_named_map(
    state_ptr: *world_state.WorldState,
    id_ptr: [*]const u8,
    id_len: u32,
) u32 {
    const id = id_ptr[0..id_len];
    const m = maps_gen.findNamedMap(id) orelse return 0;

    const count: u32 = @intCast(@min(m.statics.len, world_state.MAX_STATICS));
    var i: u32 = 0;
    while (i < count) : (i += 1) {
        state_ptr.statics[i] = m.statics[i];
        state_ptr.one_way[i] = m.one_way[i];
    }
    state_ptr.static_count = count;

    const spawns: u32 = @intCast(@min(m.spawns.len, MAX_SPAWN_POINTS));
    var s: u32 = 0;
    while (s < spawns) : (s += 1) {
        g_spawn_points_x[s] = m.spawns[s].x;
        g_spawn_points_y[s] = m.spawns[s].y;
    }
    g_spawn_point_count = spawns;

    g_arena_size_x = m.width;
    g_arena_size_y = m.height;

    return count;
}

/// How many named maps the core carries — lets a shell enumerate without
/// hardcoding the list, and lets a test assert the codegen ran.
pub export fn named_map_count() u32 {
    return @intCast(maps_gen.NAMED_MAPS.len);
}

/// Parity-test export, mirroring `gen_arena_geometry`'s shape for named
/// maps: writes [static_count, (x,y,w,h,one_way)×N, spawn_count, (x,y)×M,
/// width, height] as f64s and returns how many were written. 0 = unknown id.
///
/// Exists so the TS parity test can compare against `buildStaticCache`
/// without reimplementing WorldState struct offsets in JS — the same
/// reasoning that produced gen_arena_geometry.
pub export fn named_map_geometry(id_ptr: [*]const u8, id_len: u32, out: [*]f64) u32 {
    const m = maps_gen.findNamedMap(id_ptr[0..id_len]) orelse return 0;
    var i: usize = 0;
    out[i] = @floatFromInt(m.statics.len);
    i += 1;
    for (m.statics, 0..) |a, k| {
        out[i] = a.x;
        out[i + 1] = a.y;
        out[i + 2] = a.w;
        out[i + 3] = a.h;
        out[i + 4] = @floatFromInt(m.one_way[k]);
        i += 5;
    }
    out[i] = @floatFromInt(m.spawns.len);
    i += 1;
    for (m.spawns) |s| {
        out[i] = s.x;
        out[i + 1] = s.y;
