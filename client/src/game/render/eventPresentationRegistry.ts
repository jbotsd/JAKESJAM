...
export const EVENT_PRESENTATION_REGISTRY = {
  ...
  "resonance-triggered": {
    ...
    state: "complete",
    note: "Track L: StatusVfxController draws the linked sapphire ring-pair at the cast site (the chain, drawn — sapphirePulse, the class-agnostic ability-system accent) on top of SimEventRouter's heavy card audio + local shake; the open window itself also reads on the body via statusWindowPlan's quiet orbiting resonance tick.",
  },
  "contagion-jump": {
    action: "Contagion copies a burn from a burning enemy onto a fresh nearby enemy",
    stateChange: "The curse visibly crawls from the source body to its new host",
    lead: "world-vfx",
    channels: ["world-vfx", "particles", "world-state"],
    intensity: "hit",
    lowTierCore: "Source-to-target arc direction remains visible",
    state: "complete",
    note: "Track L: emitted at the pendingSyzygistCasts COMMIT site (never claims a rejected jump); StatusVfxController draws a sagging fire-tinted thread source→target (burn register — a curse crawling, deliberately not lightning's jitter), and the state-driven burn sparks starting on the new host carry the continuation. Silent by design (never-synthesize; the cast press's card cue already fired).",
  },
  "emission-leech": {
