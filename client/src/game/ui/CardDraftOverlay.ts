...
import { buildClassPicker, type ClassRowConfig } from "./classPicker.ts";
// Re-exported so existing importers keep their import site (Doors 1.8).
export type { ClassOption, ClassRowConfig } from "./classPicker.ts";
...
/**
 * Class-select row (classes-goal.md P1 — the loadout station is the
 * pre-arena identity moment, so class select joins card select in ONE
 * overlay; no second totem). Options carry the sim-stable archetype id
 * plus the class-era display layer. All four chassis are selectable in P1:
 * they select today's stat-archetypes under their class names — summaries
 * must describe what's true TODAY, the quiet "full kit soon" tag is the
 * only nod to the future (no false advertising).
 */
...
export class CardDraftOverlay {
  ...
  /**
   * Compact 4-chassis selector (classes-goal.md P1). Pure DOM, drawn
   * sigils only (inline SVG — crystal/diamond grammar, instrument-read; no
   * Providence compositions, no eye-under-triangle). Selection highlight is
   * state-driven, mirrored to `aria-pressed`; clicking a tile re-highlights
   * immediately and hands the archetype id to the caller to persist.
   */
  /** Delegates to the shared picker (Doors 1.8) — the station's appearance
...
/**
 * Drawn class sigils — crystal/diamond grammar, stroke-only, instrument-
 * read (art-direction discipline: simple marks, NO Providence compositions,
 * no eye-under-triangle). currentColor so the tile owns the tint.
 *   wizard  = crystal shard (the munitions the whole kit already fires)
 *   ninja   = twin slashes
 *   paladin = board/kite silhouette
 *   priest  = radiant ring
 */
