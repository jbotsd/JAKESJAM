...
## Native drafting — algorithm ported and proven, wiring not started

`sim/src/draft.zig` (new) ports the offer-generation ALGORITHM from
...
the `unique`/`maxStacks` candidate-pool filter. This is proven parity, not
asserted parity: `sim/tools/gen_card_data.ts` now also emits `rarity`,
`catch_up_eligible` (precomputed set-membership against
`CATCH_UP_BUCKETS`, since `weightForCard` never needs the full bucket list,
only that test), `unique`, and `max_stacks` per card into
`cards_gen.zig` — real metadata that didn't exist in the Zig data layer
before today. Ten new tests in `sim/test/smoke.zig` check Zig's
`rollOffers`/`weightForCard`/`poolIndices` against **golden vectors
generated by actually running the real TS `enterDrafting` loop** (five
seeds, three roles, a tight 4-card pool, and a degenerate 1-card pool) —
every offered card index and every final RNG cursor value in those tests
came out of the real TS code, not out of a spec or an assumption. All pass.
One structural invariant fell out of this and is now asserted directly:
`pickWeighted` draws exactly one `nextFloat` per call regardless of
weights, so the RNG cursor evolves identically for `catch_up` and `winner`
roles given the same pool size and seed, even though the *picks* differ —
confirmed against TS, not coincidence.

**What this does NOT do**: wire into the round-phase machine. There is no
`WorldState` field for `draftingOffers`/`draftingPicked`/
`draftingExpiresAtTick`, no phase transition from `round-over` into
`drafting` in `world.zig`, and no host-callable pick-commit export.
`round.zig`'s `roundStepPhase` stays a pure phase-timer function with no
score/drafting awareness, by its own existing design (the orchestrator
was always meant to wrap it with that bookkeeping). The server's
`applyDraftingOverlay` TS path is what actually makes drafting work in
production today, unaffected by any of this. This was a deliberate scope
cut: the wire-format + phase-transition + networking work is substantial
and was judged lower priority than the live-correctness bugs already fixed
above, since the server overlay already makes drafting fully functional —
this gap is architectural self-containment for `step_world`, not a live
correctness bug.

## Not attempted this session

- **Wiring native drafting into the round-phase machine** — see above; the
  algorithm is ported and proven, the WorldState fields/phase-transition/
  networking to actually use it in `step_world` are not.
...
- **Native drafting in `round.zig`/`world.zig`** — the offer-generation
  algorithm is now ported and proven byte-for-byte against real TS output
  (`sim/src/draft.zig`, 10 golden-vector tests), and the data layer it
  needs (`unique`/`maxStacks`/`rarity`/catch-up-bucket-membership) now
  exists in `cards_gen.zig`. What's still missing is wiring: WorldState
  fields for offer/pick bookkeeping, the `round-over` → `drafting` phase
  transition in `world.zig`, and a host-callable pick-commit export. The
  round-phase machine still can't draft *end-to-end* on its own; the
  server's `applyDraftingOverlay` remains the only path in production.
  This is the last structural gap between "step_world is a complete,
  self-contained replacement for stepWithRuntime" and where it stands
  today — smaller than it was at the start of this session, not closed.
