...
## Native drafting — algorithm, wire format, and phase machine all wired end-to-end

`sim/src/draft.zig` ports the offer-generation ALGORITHM from
`draftWeights.ts` (`classifyDraftRole`/`weightForCard`/`pickWeighted`) and
the core weighted-retry loop from `round.ts`'s `enterDrafting` (the
`while (offered.length < target && attempts < target*8)` block), including
the `unique`/`maxStacks` candidate-pool filter. `sim/tools/gen_card_data.ts`
emits `rarity`, `catch_up_eligible` (precomputed set-membership against
`CATCH_UP_BUCKETS`, since `weightForCard` never needs the full bucket list,
only that test), `unique`, and `max_stacks` per card into `cards_gen.zig` —
metadata that didn't exist in the Zig data layer before this session. Ten
tests in `sim/test/smoke.zig` check Zig's `rollOffers`/`weightForCard`/
`poolIndices` against **golden vectors generated by actually running the
real TS `enterDrafting` loop**; every offered card index and RNG cursor
value came out of real TS execution, not a spec.

That algorithm is now WIRED into the round-phase machine, end to end:

- `world_state.zig`: `PlayerEntity` grew 288→312 bytes (`card_ids: [8]u16`
  — the actual held-card list, which is genuinely new: before this,
  step_world had no channel for a player's raw hand at all, only a
  pre-resolved numeric `ResolvedFireConfig`; plus `draft_offers`,
  `draft_offer_count`, `draft_picked_offer`). `WorldStateHeader` grew
  48→56 (`round_winner_idx`, `drafting_expires_at_tick`).
- `world.zig`: `round-over` → `drafting` entry (`enterDraftingNative`,
  parity port of `enterDrafting`), the match-already-won "stay parked"
  override (a real gap round.zig's pure timer alone can't express — the
  orchestrator has to intercept it, same as drafting entry), the
  orchestrator-driven `drafting` → `countdown` exit gate (all-picked OR
  expired, expiry auto-picks offer 0 for stragglers), and a
  host-callable `world_state_commit_draft_pick` export. Five new
  Zig-native `smoke.zig` tests drive the REAL `stepWorld` orchestrator
  through full round-over→drafting→countdown cycles (not just the
  isolated algorithm) and assert on phase transitions, offer rolls,
  pick-commit, auto-pick-on-expiry, and unique-card exclusion.
- `sim/src/weapon_build.zig`'s pre-existing `resolve_player_fire_config`
  export (the one per-tick channel that already carries a player's raw
  card indices into wasm, for build resolution) now ALSO persists them
  into the new `card_ids` field as a side effect — reusing an existing
  hook rather than adding a second host→wasm card-list call.
- `worldStateBridge.ts` / `worldWasmBackend.ts` / `serverWasmHost.ts`:
  full pack/unpack of every new field, a `cardIndex.ts` module (id↔index,
  shared with the existing `fireConfigShared.ts` build-resolution path so
  there's one source of truth, not two), and a `commitDraftPick()`
  wrapper.
- `client/src/sim/wasm/__tests__/nativeDraftingE2E.test.ts` (new): drives
  the REAL TS `packWorldState` → `step_world` → `unpackWorldState` cycle
  across multiple ticks (not Zig in isolation) and asserts the rolled
  offers are byte-identical to an independent TS-side computation of
  `enterDrafting` for the same seed — proving the full wire round-trip,
  not just the algorithm.

**Three real bugs were found and fixed while wiring this, all with a
failing test first**:
1. `packPlayer` originally hardcoded `draft_offers`/`draft_picked_offer`
   to a "not drafting" sentinel on every pack, instead of round-tripping
   `PlayerEntity.draftOffers`/`draftPickedOffer`. Since every
   `applyWasmWorldStepFull` call re-packs from the TS state object, this
   would have silently reset any drafting window to "nobody's drafting"
   on the second tick — caught by the `nativeDraftingE2E.test.ts`
   "survives a second pack/unpack cycle" regression test, which fails
   without the fix and passes with it.
2. Both `worldWasmBackend.ts`'s and `serverWasmHost.ts`'s `mergeUnpacked`
   never threaded `winnerPlayerId`/`draftingExpiresAtTick` from the
   freshly unpacked result — they fell through the `...state.round`
   spread's stale pre-step value instead. `draftingExpiresAtTick` landing
   at `undefined` → packed as `0` → `tick >= 0` is always true → drafting
   collapsed back to `countdown` on the very next tick regardless of
   whether anyone had picked. Same regression test caught this too.
3. Growing `PLAYER_ENTITY_SIZE`/`HEADER_SIZE` broke five `__tests__`
   files (and, more seriously, TWO production files —
   `worldWasmBackend.ts`'s `writeScoresIntoMemory`/
   `writePlayerInputsIntoMemory` and `serverWasmHost.ts`'s equivalents)
   that had hand-copied the old byte offsets instead of importing them.
   Fixed by exporting the layout constants from `worldStateBridge.ts` and
   updating every call site to derive from them.

**What's still genuinely open**: no UI/networking wiring
(`CardDraftOverlay` doesn't call `commitDraftPick`; that's expected —
`USE_WASM_STEP_WORLD` is off and the server's `applyDraftingOverlay` TS
path is what actually runs drafting in production today, untouched by any
of this). A native pick's grant reaching a caller's OWN canonical player
record depends on that caller threading the returned state forward — true
for every existing `applyWasmWorldStepFull` caller today, but worth
re-confirming if a future session wires this into `matchHost.ts` for real.

## Not attempted this session

- **Wiring native drafting into `CardDraftOverlay`/networking** — the
  phase machine, wire format, and algorithm are done and proven; nothing
  in the client UI or server match loop calls `commitDraftPick` yet.
...
- **Native drafting's UI/networking wiring** — the algorithm
  (`sim/src/draft.zig`, 10 golden-vector tests), the data layer
  (`unique`/`maxStacks`/`rarity`/catch-up-bucket-membership in
  `cards_gen.zig`), the wire format (`card_ids`/`draft_offers`/
  `draft_picked_offer`/`round_winner_idx`/`drafting_expires_at_tick`),
  and the phase machine (`round-over` → `drafting` → `countdown`,
  pick-commit export, expiry auto-pick) are ALL wired and proven — both
  in Zig isolation (`smoke.zig`) and through the full TS↔wasm wire round
  trip (`nativeDraftingE2E.test.ts`, including two real multi-tick bugs
  found and fixed via a failing regression test first). `step_world` can
  now run a complete drafting cycle on its own, byte-for-byte matching
  TS's `enterDrafting` for the same seed. What's NOT done: nothing in
  `CardDraftOverlay` or the server match loop calls `commitDraftPick` —
  the server's `applyDraftingOverlay` TS path remains the only thing
  actually running drafting in production. That's the last gap between
  "step_world is a complete, self-contained replacement for
  stepWithRuntime" and where it stands today — UI/networking integration,
  not simulation correctness.
