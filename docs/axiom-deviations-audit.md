...
### Kindred (paladin) — two structural gaps [NEW]
- **[NEW] Coverage-lock violation:** the file's own lock requires **≥2 primary tags per
  role**, but Kindred's catalog has **buff ×1 (Rally Light) and movement ×1 (Plant Charge)**.
...
- **[NEW] Solo-dead abilities (A8 / coverage):** **Aegis Share (#8)** and **Rally Light (#9)**
  ...
  solo fallback; Kindred was not. Fix: give Aegis/Rally a solo clause, or accept Kindred as a
...
- **Proposed fix (2026-07-18, Jake flagged "paladin doesn't have the same amount of abilities"):**
  ...
  This closes Kindred to the ≥2-per-role floor + no-solo-dead, matching Syzygist, and is the
  **prerequisite for the paladin graphical centerpiece** (`presentation-overhaul-goal.md` P1 —
  you can't world-class-render an incomplete roster). Also note the **build** gap: Kindred is
...
- **RESOLVED (2026-07-18, coverage-floor + solo-viability fast-follow):** both structural gaps
  ...
  `kindredCatalog.test.ts`'s existing "solo-safe, no teamId needed" tests, which already covered
...
- **Solo-viability — done right (affirm):** Borrowed Time / Glass Ward / Haste Gift all carry
  ...
  This is the model Kindred should copy.
- **[shares Kindred's] Coverage:** buff ×1 (Haste Gift), movement ×1 (Drift Step) — same
  ≥2-per-role gap.
...
missed (Kindred's coverage-floor miss and its solo-dead abilities), plus located the priority
